...
const authorLinkTemplate = (postAuthor) => {
    return(
        <a href={postAuthor.link} className="wp-block-example-hero__author m-0 text-background">
            {postAuthor.name}
        </a>
    )
}
    ...
    return (
        <>
            <PostContext 
                ...
                isEditable={false}
            >
                <div className={`flex flex-col-reverse gap-8 lg:flex-row ${ className }`}>
                    <div className="flex flex-col justify-center w-full lg:w-1/2 gap-2">
                        ...
                        <PostTitle tagName={ titleTag } className="wp-block-nota-de-tapa-post__title m-0 text-background" />
                        { 
                            showExcerpt
                            && <PostExcerpt 
